...
public class InquiryManagementServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request,
        ...
        if ("/admin/inquiry/detail".equals(servletPath)) {
     ...
     * 1:1 문의 목록 화면
     */
    private void getInquiryList(HttpServletRequest request,
                                HttpServletResponse response)
            throws ServletException, IOException {
        ...
        range.setStartNum(
                service.startNum(currentPage, pageScale));

        range.setEndNum(
                service.endNum(currentPage, pageScale));

        int totalCount =
                service.totalCount("all", keyword, searchDate);

        int waitingCount =
                service.totalCount("waiting", keyword, searchDate);

        int completeCount =
                service.totalCount("complete", keyword, searchDate);

        int listTotalCount =
                service.totalCount(status, keyword, searchDate);

        range.setTotalCount(listTotalCount);

        range.setTotalPage(
                service.totalPage(listTotalCount, pageScale));

        List<InquiryListDTO> inquiryList =
                service.getInquiryDashboardList(
                        range,
                        status,
                        keyword,
                        searchDate);
     ...
     * 1:1 문의 상세 JSON 조회
     */
    private void getInquiryDetail(HttpServletRequest request,
                                  HttpServletResponse response)
            throws IOException {
        ...
        String inquiryCodeParam =
                request.getParameter("inquiryCode");

        if (inquiryCodeParam != null &&
            !inquiryCodeParam.trim().isEmpty()) {

            try {
                inquiryCode =
                        Integer.parseInt(inquiryCodeParam);
            } catch (Exception e) {
                inquiryCode = 0;
            }
        }
        ...
        InquiryDetailDTO detail =
                service.getInquiryDetail(inquiryCode);
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if ("/admin/inquiry/reply".equals(servletPath)) {
     ...
     * 답변 등록 / 수정
     */
    private void replyInquiry(HttpServletRequest request,
                              HttpServletResponse response)
            throws IOException {
        ...
        if (adminIdObj == null) {
            adminIdObj =
                    request.getSession().getAttribute("loginAdminId");
        }
        ...
        String adminId =
                String.valueOf(adminIdObj);

        InquiryDetailDTO inquiry =
                new InquiryDetailDTO();
