...
<div class="layout">
    ...
    <main class="main">
			    ...
			    <tr>
			        ...
			        <td>
			            <button type="button"
						        class="edit-btn"
						        aria-label="수정"
						        onclick="openScheduleEditModal(<%= s.getGameScheduleCode() %>)">
            ...
            <table class="team-table">
                <thead>
                    <tr>
			            ...
			            <th style="width: 100px;">팀 약칭</th>
			            <th>구장</th>
			            <th style="width: 90px; text-align:center;">수정</th>
			        </tr>
                </thead>
                <tbody>
                    <%
			        ...
			        <tr>
			            <td>
			                <%
			                String teamLogoImg = t.getTeamLogoImg();
			            ...
			            <td class="team-name-cell">
			                <%= t.getTeamName() != null ? t.getTeamName() : "-" %>
			            </td>
			
			            <td class="team-short-cell">
			                <%= t.getTeamShortName() != null ? t.getTeamShortName() : "-" %>
			            </td>
			
			            <td class="team-stadium-cell">
			                <%= t.getStadiumName() != null ? t.getStadiumName() : "-" %>
			            </td>
			            ...
			            <td style="text-align:center;">
			                <button type="button"
			                        class="edit-btn"
			                        aria-label="수정"
			                        onclick="openTeamEditModal(<%= t.getTeamCode() %>)">
			                    <i class="ti ti-edit"></i>
			                </button>
			            </td>
			        </tr>
        ...
        <%@ include file="fragments/scheduleEditModal.jspf" %>
        <script>
		const teamDataMap = {
		<%
		for (TeamInfoDTO t : teamList) {
		%>
		    "<%= t.getTeamCode() %>": {
		        ...
		        teamName: "<%= t.getTeamName() != null ? t.getTeamName().replace("\\", "\\\\").replace("\"", "\\\"") : "" %>",
		        teamShortName: "<%= t.getTeamShortName() != null ? t.getTeamShortName().replace("\\", "\\\\").replace("\"", "\\\"") : "" %>",
		        stadiumCode: "<%= t.getStadiumCode() %>",
		        teamLogoImg: "<%= t.getTeamLogoImg() != null ? t.getTeamLogoImg().replace("\\", "\\\\").replace("\"", "\\\"") : "" %>"
		    },
		<%
		}
		%>
...
<%@ include file="fragments/teamModal.jspf" %>
    </main>
...
<script>
    ...
    function openScheduleModal() {
        ...
        document.getElementById('scheduleMode').value = 'insert';
        document.getElementById('gameScheduleCode').value = '';

        document.getElementById('scheduleModalTitle').textContent =
            '경기 일정 추가';

        document.getElementById('scheduleSubmitBtn').textContent =
            '일정 추가';
    ...
    function clearScheduleModal() {
        ...
        setScheduleInputValue('homeTeamCode', '');
        setScheduleInputValue('awayTeamCode', '');
        setScheduleInputValue('gameDate', '');
        setScheduleInputValue('gameStartTime', '');
        setScheduleInputValue('stadiumCode', '');
    }

    function setScheduleInputValue(id, value) {
